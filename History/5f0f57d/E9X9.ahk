...
; Chrome - checks multiple common locations
#b::RunOrFocusChrome()
...
RunOrFocusApp(winTitle, exeName, exePath)
{
    ...
    ; If we just launched the app, wait for its window to appear
    if justLaunched {
        try {
            WinWait(searchCriteria, , 5)  ; Wait up to 5 seconds
        } catch {
...
; ---------------------
#Escape::Reload  ; Win+Escape to reload the script

; ---------------------
; Chrome Helper Function
; ---------------------
RunOrFocusChrome()
{
    chromePaths := [
        ...
        A_LocalAppData "\Google\Chrome\Application\chrome.exe"
    ]
    ...
    ; If Chrome not found, just try to run it via PATH
    if chromePath = "" {
        chromePath := "chrome.exe"
    }
    
    RunOrFocusApp("", "chrome.exe", chromePath)
}